...
function Compass() {
  ...
  // ✅ Tıklama sayacı ve zaman kontrolü
  const clickCountRef = useRef(0);
  const lastClickAtRef = useRef(0);
  const REQUIRED_CLICKS = 4;   // üst üste kaç tıklamada tetiklenecek
  const MAX_INTERVAL_MS = 800; // tıklamalar arası maksimum ms

  const handleIconClick = () => {
    const now = Date.now();

    if (now - lastClickAtRef.current > MAX_INTERVAL_MS) {
            ...
            <ItemTitle>
              <PiCompassRoseFill
                ...
                style={{
                  ...
                  transition: 'transform 0.3s linear',
                  cursor: 'pointer', // kullanıcı tıklayabileceğini görür
                }}
                onClick={handleIconClick} // ✅ tıklama olayını bağladık
              />
            </ItemTitle>
          </ItemContent>
        </Item>
